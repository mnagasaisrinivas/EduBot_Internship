...
class AIAPI {
  ...
  async generatePracticeProblems(data: PracticeProblemsRequest): Promise<PracticeProblemsResponse> {
    const response = await this.axiosInstance.post('/api/practice/', data);
    return response.data;
  }

  async generateStudyNotes(data: StudyNotesRequest): Promise<StudyNotesResponse> {
